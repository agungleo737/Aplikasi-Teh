    ...
    private void simpanProduk() {
        ...
        if (uriThumb != null && uriFull != null) {
            try {
                ...
                call = apiService.tambahProduk(partThumb, partFull, rbNama, rbKategori, rbHarga, rbStok, rbDeskripsi);
            } catch (Exception e) {
                Toast.makeText(this, "Gagal proses foto: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                return;
            }
        } else {
            call = apiService.tanpagambar(rbNama, rbKategori, rbHarga, rbStok, rbDeskripsi);
        }
